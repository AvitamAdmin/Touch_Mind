...
public abstract class ApiCommonService {
            ...
            if (reportDto.getCurrentNode() != null) {
                if (indexId != null) {
                    if (StringUtils.isNotEmpty(relationId)) {
                        entities = genericRepository.findByIndexIdAndIdentifierAndRelationIdAndSessionId(indexId, recordId + reportDto.getCurrentSite(), relationId, reportDto.getCurrentSessionId());
                    } else {
                        entities = genericRepository.findByIndexIdAndIdentifierAndSessionId(indexId, recordId + reportDto.getCurrentSite(), reportDto.getCurrentSessionId());
                    }
                } else {
                    if (StringUtils.isNotEmpty(relationId)) {
                        entities = genericRepository.findByIdentifierAndRelationIdAndSessionId(recordId + reportDto.getCurrentSite(), relationId, reportDto.getCurrentSessionId());
                    } else {
                        entities = genericRepository.findByIdentifierAndSessionId(recordId + reportDto.getCurrentSite(), reportDto.getCurrentSessionId());
                    }
                }

            } else {
                //TODO check this if still saving correctly
                entities = genericRepository.findByIdentifier(recordId);
    ...
    public Map<String, String> getRelationKeysForDatasource(DataSource dataSource, String node) {
        ...
        Optional<DataRelationParams> dataRelationParamsOptional = dataRelations.stream().filter(dataRelationParams -> {
            DataSource ds = dataSourceRepository.findByIdentifier(dataRelationParams.getDataSource());
            return StringUtils.equals(ds.getIdentifier(), dataSource.getIdentifier());
        }).findFirst();
