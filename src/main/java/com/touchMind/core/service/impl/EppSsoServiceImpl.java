...
//    private SubsidiaryRepository subsidiaryRepository;
    ...
    public EppSso generateSsoLink(String site, EppSsoWsDto eppSsoWsDto) throws ParseException {
        Site siteModel = siteRepository.findByIdentifier(site);
        ...
        if (StringUtils.isNotEmpty(eppSsoWsDto.getEnvironment())) {
            Environment environment = environmentRepository.findByIdentifier(eppSsoWsDto.getEnvironment());
            if (environment != null) {
      ...
      //  Subsidiary subsidiary1 = subsidiaryRepository.findByIdentifier(subsidiary);
        String md5Link = getEppShopSsoLink +  MULTISTORE + siteModel.getSiteChannel() + SLASH + siteModel.getIdentifier() + LOGIN_REWARDSPARTNER_AFFILIATENAME +
