...
public class EnvironmentAction implements ElementActionService {
    ...
    public ActionResult performAction(ActionRequest actionRequest) {
        ...
        try {
            ...
            Environment environment = environmentRepository.findByIdentifier(environmentId);
            if (null != environment) {
    ...
    private void performElementAction(ITestContext context, String uiSelector, String inputData, String shortDescription, boolean takeAScreenshot) {
        ...
        ThreadTestContext threadTestContext = (ThreadTestContext) context.getAttribute(TestDataUtils.Field.THREAD_CONTEXT.toString());
        ReportUtils.info(context, String.format(OPENED_MESSAGE, uiSelector, shortDescription), StringUtils.EMPTY, false);
        LocatorSelectorDto locatorSelectorDto = new LocatorSelectorDto();
        locatorSelectorDto.setXpathSelector(uiSelector);
        WebElement element = selectorService.getBy(threadTestContext, locatorSelectorDto, "EnvironmentAction", context);
        if (element == null) {
            ReportUtils.info(context, String.format(OPENED_MESSAGE, uiSelector, shortDescription), StringUtils.EMPTY, takeAScreenshot);
            return;
        }
        element.clear();
    ...
    private void performSubmitAction(ITestContext context, EnvironmentConfig config, boolean takeAScreenshot) {
        ...
        WebElement element = selectorService.getBy(threadTestContext, locatorSelectorDto, "EnvironmentAction", context);
        if (element == null) {
            ReportUtils.info(context, String.format(OPENED_MESSAGE, config.getActionElement(), config.getShortDescription()), StringUtils.EMPTY, takeAScreenshot);
            return;
        }
        ReportUtils.info(context, String.format(OPENED_MESSAGE, config.getActionElement(), config.getShortDescription()), StringUtils.EMPTY, takeAScreenshot);
