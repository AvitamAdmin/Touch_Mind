...
public class SecurityController extends BaseController {
    ...
    public UserWsDto processRegister(@RequestBody UserWsDto userWsDto) {
        ...
        eventPublisher.publishEvent(new OnRegistrationCompleteEvent(user, appUrl, "New user registration", user.getUsername() + MAIL_CONTENT, "hybris.sup@touchMind.com", "1", EMAIL_CONTENT2));
    ...
    public String confirmRegistration(@RequestBody UserDto userDto) throws UnsupportedEncodingException {
        ...
        if (result != null && result.equals("valid")) {
            ...
            if (level.equals("1")) {
                eventPublisher.publishEvent(new OnRegistrationCompleteEvent(user, appUrl, "New user Registration", user.getUsername() + MAIL_CONTENT, user.getReferredBy(), "2", EMAIL_CONTENT2));
                return "User Approved";
            } else if (level.equals("2")) {
                eventPublisher.publishEvent(new OnRegistrationCompleteEvent(user, appUrl, "Registration Successful", "Registration successful, Kindly click link below to verify your account", user.getUsername(), "3", EMAIL_CONTENT2));
                return "User Approved";
