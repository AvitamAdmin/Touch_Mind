              ...
              <tbody>
                  <c:forEach items="${models}" var="model">
                    <tr id="${model.id}">
                        ...
                        <td class="td-sm">${model.cronId}</td>
                        <td class="td-sm">${model.cronExpression}</td>
                        <td class="td-sm">${model.mapping}</td>
                        <td class="td-sm">${model.subsidiary}</td>
                        <td class="td-sm">${model.jobStatus}</td>
                       <c:choose>
                           <c:when test="${model.jobStatus=='Stopped'}">
                               <td class="td-sm"><button style="background: green !important;" class="btn btn-primary btn-cheil" onclick="fire_ajax_submit('/admin/scheduler/startJob/${model.id}', 'false')" type="button">Start</button></td>
                           </c:when>
                           <c:otherwise>
                               <td class="td-sm"><button style="background: red !important;" class="btn btn-primary btn-cheil" onclick="fire_ajax_submit('/admin/scheduler/stopJob?id=${model.id}&delete=false', 'false')" type="button">Stop</button></td>
                           </c:otherwise>
                       </c:choose>
