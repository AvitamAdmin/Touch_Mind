                            ...
                            <c:forEach items="${label.children}" var="child">
                                  <c:choose>
                                      <c:when test="${unselectedChild eq child}">
                                        <option value="${fn:replace(unselectedChild,"'","")}" selected>${fn:replace(unselectedChild,"'","")}</option>
                                      </c:when>
                                      <c:otherwise>
                                         <option value="${fn:replace(unselectedChild,"'","")}" >${fn:replace(unselectedChild,"'","")}</option>
                                      </c:otherwise>
                                  </c:choose>
                              </c:forEach>
                          </c:forEach>
    ...
    $(document).ready(function() {
      var count = $('#existingLabelCount').val();
            $(".add-label-more").click(function(){
                var html = '<div class="row" id="label'+count+'"><div class="col-sm-4"><select class="3col active cheil-select" name="labels['+count+'].parent" id="parentSelectpicker" ><option value="">Select label</option><c:forEach items="${labels}" var="child"><option value="${fn:replace(child,"'","")}" >${fn:replace(child,"'","")}</option></c:forEach></select></div><div class="col-sm-4"><select class="3col active cheil-select" name="labels['+count+'].children" id="selectpicker-'+count+'" multiple><option value="">Select child labels</option><c:forEach items="${labels}" var="unselectedChild"><option value="${fn:replace(unselectedChild,"'","")}" >${fn:replace(unselectedChild,"'","")}</option></c:forEach></select></div><div class="col-sm-4"><img style="width:32px;height:32px;margin-left:25px;margin-bottom:10px;" onclick="javascript:removeToolkitRow('+count+')" src="${contextPath}/images/remove.png" /></div></div><br/><br/>';
