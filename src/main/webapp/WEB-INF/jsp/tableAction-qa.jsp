...
var table = $('#tableData').DataTable({
    ...
    columns : [
        ...
        { "data": "testPassedCount",
            "render":function(currentData,type,row) {
                if (row.failedSkusError){
                   return '<a href="/qa/failedSkus?id='+row.DT_RowId+'" target="_blank" style="color:blue;">'+row.testPassedCount+'/'+(row.testFailedCount+row.testPassedCount)+' ('+Math.round(row.testPassedCount/(row.testFailedCount+row.testPassedCount)) * 100+'%)</a>';
                } else {
                   return row.testPassedCount+'/'+(row.testFailedCount+row.testPassedCount)+' ('+ Math.round(row.testPassedCount/(row.testFailedCount+row.testPassedCount)) * 100+'%)';
                }
            }
        },
        { "data": "resultStatus",
    ...
    buttons: [
        ...
        {
              text: '<button class="btn btn-primary btn-icon" id="repairBtn" type="button" disabled="disabled">Repair</button>',
             titleAttr: "Repair",
              action: function ( e, dt, node, config ) {
              var value = $("#rowSelectorId").val();
                  if(value) {
                    fire_ajax_submit("/"+$("#navBreadcrumb").text().split(" > ")[1]+"/"+$("#navBreadcrumb").text().split(" > ")[2]+"/repair?id="+$("#rowSelectorId").val());
                  }
                   else {
                    alert('Please select the row you wanted to edit! ');
                  }
      ...
      if ($(this).hasClass('selected')) {
              ...
              console.log(selectedIds);
                        ...
                        if(length==1)
                        {
                          ...
                          $('#repairBtn').removeAttr('disabled');
                          $('#deleteBtn').removeAttr('disabled');
                        }
                        if(length>1)
                          {
                            $('#editBtn').attr('disabled', 'disabled');
                            $('#repairBtn').attr('disabled', 'disabled');
                            $('#deleteBtn').removeAttr('disabled');
                          }
