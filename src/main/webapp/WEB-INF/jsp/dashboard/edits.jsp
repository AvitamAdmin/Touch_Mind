...
<div class="main-content">
    ...
    <form:form method="POST" id="editForm" enctype="multipart/form-data" action="/admin/action/edit" class="handle-upload" modelAttribute="editForm" >
        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                <form:input path="description" class="inputbox-cheil" placeholder="Enter Description" readonly="true" />
                    ...
                    <div class="col-sm-12">
                          <select name="subAndSites[]" placeholder="Select subsidiaries" multiple="multiple" id="selectpicker8" disabled="disabled">
                            ...
                            <optgroup label="${child.key}">
                              ...
                              <c:choose>
                                    <c:when test="${fn:contains( editForm.sites, site.siteId ) }">
                                      <option value="${child.key}-${site.siteId}" selected>${site.siteId}</option>
                                    </c:when>
                                    <c:otherwise>
                                       <option value="${child.key}-${site.siteId}" >${site.siteId}</option>
                                    </c:otherwise>
                                </c:choose>
                              </c:forEach>
                               </optgroup>
              ...
              <div class="col-sm-3">
                              ...
                              <option value="">Select role </option>
                                  <c:forEach items="${roles}" var="role">
                                      <c:choose>
                                          <c:when test="${editForm.role == role.id}">
            ...
            <div class="col-sm-3">
                 ...
                 <option value="">Select catalog</option>
                    <c:forEach items="${catalogs}" var="child">
                        <c:choose>
                            <c:when test="${editForm.catalogId eq child.id}">
                              <option value="${child.id}" selected>${child.shortDescription}</option>
                            </c:when>
                            <c:otherwise>
                               <option value="${child.id}" >${child.shortDescription}</option>
              ...
              <div class="col-sm-3">
                   ...
                   <option value="">Select module</option>
                      <c:forEach items="${modules}" var="child">
                          <c:choose>
                              <c:when test="${editForm.moduleId eq child.id}">
                                <option value="${child.id}" selected>${child.shortDescription}</option>
                              </c:when>
                              <c:otherwise>
                                 <option value="${child.id}" >${child.shortDescription}</option>
